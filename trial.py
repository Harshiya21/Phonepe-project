...
if select_options == "INSIGHTS":
    st.write(select_options)
    questions= st.selectbox("Select all questions",("1.Top 10 State Amount Transactions done by using Phonepe",
                                                "2.In Year 2023 and Quarter 1 Least 5 Mobile Brands has done Phonepe Total Transcations",
                                                #"3.Top 10 Districts based on the Transaction Amount",
                                                "4.Least 10 Districts based on the Transaction Amount",
                                                #"5.State based on Transaction Count",
                                                #"6.States based on Transaction Amount",
                                                "7.Least 5 States who hasn't users Phonepe in INDIA",
                                                "8.Least 10 Districts PhonePe users and App openers",
                                                #"9.Top 10 RegisteredUsers by Districts",
                                                "10.List out the year had Zero App opens on Phonepe"))
    
    if questions == "1.Top 10 State Amount Transactions done by using Phonepe":
        mycursor.execute('USE phonepae')
    ...
    elif questions == "2.In Year 2023 and Quarter 1 Least 5 Mobile Brands has done Phonepe Total Transcations":
        mycursor.execute('USE phonepae')
        mycursor.execute('''SELECT State,Year,Quarter,Brand,SUM(Transaction_count) AS Total Transactions \
        FROM Agg_Trans \
        where Year = 2023 and Quarter = 1 \
        GROUP BY State, Year, Quarter, Brand \
        ORDER BY Total Transactions ASC \
        LIMIT 5''')
        df2=pd.DataFrame(mycursor.fetchall(),columns=["State","Year","Quarter","Brand","Total Transactions"])
        fig_bar_2 =px.bar(df2,x=["State","Year","Quarter","Brand"],y="Total Transactions",title="Least 5 Total Transaction by State",
                        width=600,color_discrete_sequence=px.colors.sequential.Jet)
        st.plotly_chart(fig_bar_2)

    elif questions == "3.Top 10 Districts based on the Transaction Amount":
        mycursor.execute('USE phonepae')
        ...
        ORDER BY Transaction_Amount DESC \
        LIMIT 10''')
        df3=pd.DataFrame(mycursor.fetchall(),columns=["District","Transaction_Amount"])
        fig_line_3 = px.line(df3, x="District", y="Transaction_Amount",title ="Top 10 Districts by Transaction Amount", width=1000, height=800,
                        markers=True, color_discrete_sequence=px.colors.sequential.Magenta_r)
        st.plotly_chart(fig_line_3)    

    elif questions == "4.Least 10 Districts based on the Transaction Amount":    
        mycursor.execute('USE phonepae')
        mycursor.execute('''SELECT District,SUM(Transaction_amount) AS Transaction_Amount \
        FROM Map_Trans \
        GROUP BY District \
        ...
        LIMIT 10''')
        df4=pd.DataFrame(mycursor.fetchall(),columns=["District","Transaction_Amount"])
        fig_line_4 = px.line(df4, x="District", y="Transaction_Amount",title ="Least 10 Districts by Transaction Amount", width=1000, height=800,
                        markers=True, color_discrete_sequence=px.colors.sequential.Viridis)
        st.plotly_chart(fig_line_4)

    elif questions == "5.State based on Transaction Count":    
        mycursor.execute('USE phonepae')
    ...
    elif questions == "6.States based on Transaction Amount":
        mycursor.execute('USE phonepae')
        ...
        fig_bar_6 = px.bar(df6, x="State", y="Transaction_Amount",title="State by Transaction_Amount",
                        width=600,color_discrete_sequence=px.colors.sequential.Greens_r)
        st.plotly_chart(fig_bar_6)  

    elif questions == "7.Least 5 States who has users Phonepe in INDIA":
        mycursor.execute('USE phonepae')
        ...
        LIMIT 5''')
        df7=pd.DataFrame(mycursor.fetchall(),columns=["State","Brand","Transaction_Count"])
        fig_sunburst_7 = px.sunburst(df7, path=["State","Brand"], values="Transaction_Count")
        st.plotly_chart(fig_sunburst_7)

    elif questions == "8.Least 10 Districts PhonePe users and App openers":
        mycursor.execute('USE phonepae')
        mycursor.execute('''SELECT Districts,SUM(AppOpens) AS AppOpens,SUM(RegisteredUsers) AS RegisteredUsers \
        FROM Map_Users \
        GROUP BY Districts,RegisteredUsers \
        ORDER BY AppOpens ASC \
        LIMIT 10''')
        df8=pd.DataFrame(mycursor.fetchall(),columns=["Districts","AppOpens","RegisteredUsers"])
        fig_pie_8=px.pie(df8, names=["Districts","RegisteredUsers"], values="AppOpens",width=600,hole=0.5)
    ...
    elif questions == "10.List out the year had Zero App opens on Phonepe":
        mycursor.execute('USE phonepae')
        mycursor.execute('''SELECT DISTINCT(Year),State,AppOpens AS AppOpens \
        FROM Map_Users \
        WHERE AppOpens = 0 
        GROUP BY State,Year''')
        df10=pd.DataFrame(mycursor.fetchall(),columns=["State","Year","AppOpens"])
        fig_pie_10 = px.pie(df10, names=["State","Year"], values="AppOpens",width=600,hole=0.5)
        st.plotly_chart(fig_pie_10) 
